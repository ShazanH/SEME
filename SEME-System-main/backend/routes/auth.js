...
router.post('/createuser', (req, res, next) => {
    console.log("Hit /createuser route");
    next();
}, [
    ...
    if (!errors.isEmpty()) {
        return res.status(400).json({ success, msg: "Validation Error", error: errors.array() })
    }
    ...
    try {
        ...
        console.log("Checked if user exists:", user);
...
router.post('/login', [
    ...
    if (!errors.isEmpty()) {
        success = false
        return res.status(400).json({ success, msg:"Validtion Error", error: errors.array() });
    }
